...
// ── 공유 드라이브(Shared Drive) 필수 처리 ─────────────────────────────────
// ⚠️ Service Account 는 개인 My Drive 에 "파일" 을 업로드할 수 없음 (storage quota 0).
//    폴더 생성은 quota 를 안 먹어 성공하지만 파일 업로드는 전부 실패함 →
//    2026-04~05 사이 모든 자동 백업이 "빈 월 폴더만 생성, .gz 0건" 으로 조용히 실패함
//    (에러는 result.errors 로 잡혀 Railway 콘솔에만 찍혀 아무도 못 봄).
//    해결: SA(jeisha-backup-uploader)가 멤버인 공유 드라이브 안 백업 폴더로 리다이렉트
//          + 모든 Drive 호출에 공유 드라이브 3종 옵션 부착 (blueprint-drive.js 와 동일 패턴).
const DRIVE_SHARED_OPTS = { supportsAllDrives: true, includeItemsFromAllDrives: true, corpora: 'allDrives' };
const LEGACY_MYDRIVE_BACKUP_ID = '18mI6WR_KmXDWKM4freDc0flh7kHHdQDU'; // Owen 개인 My Drive — SA 파일 업로드 불가
const SHARED_DRIVE_BACKUP_ID = '13rINQA7v172DRyGzBKraiPDpluSVPo8d';   // 공유 드라이브 0APdzDsYkTfv1Uk9PVA 안 "Claude cowork backups (shared)"
function resolveBackupRoot() {
  const env = process.env.BACKUP_DRIVE_FOLDER_ID;
  // env 미설정 또는 옛 My Drive ID 면 공유 드라이브 폴더로 강제 리다이렉트
  if (!env || env === LEGACY_MYDRIVE_BACKUP_ID) return SHARED_DRIVE_BACKUP_ID;
  return env;
}
...
async function ensureDriveFolder(drive, parentId, name) {
  ...
  const list = await drive.files.list({ q, fields: 'files(id,name)', pageSize: 1, ...DRIVE_SHARED_OPTS });
...
async function ensureBackupSubfolders(drive, projectName, kind, dateObj) {
  ...
  if (!root) throw new Error('백업 루트 폴더 ID 없음 (BACKUP_DRIVE_FOLDER_ID 또는 기본 공유 드라이브).');
...
function scheduleDailyBackup(opts) {
  ...
  if (!resolveBackupRoot()) {
    console.warn(`[backup] ${project} 백업 루트 폴더 없음 — 자동 백업 비활성`);
    return;
  }
  loop();
}
