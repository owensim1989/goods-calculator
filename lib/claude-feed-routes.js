// lib/claude-feed-routes.js
// 🤖 Claude 잡 데몬용 읽기전용 피드 — 파이프라인 단계전환 이벤트
// 맥미니 claude-jobs 데몬이 폴링해 "단계전환 준비물 잡"의 데이터 소스로 사용.
//
// Mount: server.js → app.use('/api/claude-feed', require('./lib/claude-feed-routes').router());
// 인증: 세션 인증 아님 — auth.js isPublicPath 에 /api/claude-feed/ prefix 등록 후
//       헤더 X-Claude-Feed-Key === process.env.CLAUDE_FEED_KEY 로 자체 가드.
//       env 미설정 시 무조건 403 (fail-closed). 읽기 전용 (GET only).
//
// GET /api/claude-feed/pipeline-events?since=<ISO>
//   since 이후의 단계 전환 이력 (pipeline-store history ev='stage' append 재사용).
//   since 없으면 최근 24시간. 최대 50건, 시간 오름차순.
//   응답: { items:[{ projectId, projectName, track, fromStage, toStage, at, by, checklist, vendors }] }

const express = require('express');
const security = require('./security'); // 2026-08-18 보안: 상수시간 키 비교

const MAX_ITEMS = 50;
const DEFAULT_WINDOW_MS = 24 * 60 * 60 * 1000; // 24h

// 공통 가드 — fail-closed
function requireFeedKey(req, res, next) {
  const expected = process.env.CLAUDE_FEED_KEY;
  if (!expected) return res.status(403).json({ error: 'feed_disabled' });          // env 미설정 → 무조건 403
  if (!security.keyEq(req.headers['x-claude-feed-key'], expected)) { // 2026-08-18 보안: 상수시간
    return res.status(403).json({ error: 'forbidden' });
  }
  next();
}

function router() {
  const store = require('./pipeline-store');
  const r = express.Router();
  r.use(requireFeedKey);

  // STAGE_META 라벨("💡 기획") → stage key("plan") 역매핑 — history detail 파싱용
  const labelToKey = {};
  for (const [key, m] of Object.entries(store.STAGE_META)) {
    labelToKey[`${m.emoji} ${m.label}`] = key;
  }
  const toStageKey = (label) => labelToKey[label] || label; // 미매칭 시 원문 유지

  // history stage detail: "💡 기획 → 🎨 디자인 (수동)" → { from, to }
  function parseStageDetail(detail) {
    const parts = String(detail || '').split(' → ');
    if (parts.length !== 2) return null;
    const from = parts[0].trim();
    const to = parts[1].replace(/\s*\([^)]*\)\s*$/, '').trim(); // 꼬리 "(수동)"/"(입고 웹훅)" 제거
    return { from: toStageKey(from), to: toStageKey(to) };
  }

  r.get('/pipeline-events', (req, res) => {
    let sinceMs;
    if (req.query.since != null && req.query.since !== '') {
      sinceMs = Date.parse(String(req.query.since));
      if (!Number.isFinite(sinceMs)) return res.status(400).json({ error: 'invalid since (ISO 8601 필요)' });
    } else {
      sinceMs = Date.now() - DEFAULT_WINDOW_MS;
    }

    const items = [];
    const db = store.loadDb();
    for (const p of db.projects) {
      const track = p.type === 'reorder' ? '재발주' : '신제품';
      // vendors 요약 (있으면)
      const vendors = (Array.isArray(p.vendors) && p.vendors.length)
        ? p.vendors.map(v => ({
            name: v.name || '',
            status: v.status || null,
            quote_count: Array.isArray(v.quotes) ? v.quotes.length : 0
          }))
        : null;
      for (const h of (p.history || [])) {
        if (h.ev !== 'stage') continue;
        const at = Date.parse(h.at);
        if (!Number.isFinite(at) || at <= sinceMs) continue;
        const st = parseStageDetail(h.detail);
        if (!st) continue;
        const checklist = ((p.checklist && p.checklist[st.to]) || [])
          .map(c => ({ text: c.t, done: !!c.done }));
        items.push({
          projectId: p.id,
          projectName: p.name,
          track,
          fromStage: st.from,
          toStage: st.to,
          at: h.at,
          by: h.by || null,
          checklist,
          ...(vendors ? { vendors } : {})
        });
      }
    }

    items.sort((a, b) => String(a.at).localeCompare(String(b.at))); // 오름차순
    res.json({ items: items.slice(0, MAX_ITEMS) });
  });

  // GET /inventory-cost-coverage (2026-07-19)
  //   카탈로그 원가_KRW → inventory products.cost_price sync 가 실제로 반영됐는지 확인용.
  //   inventory 조회는 PARTNER_API_KEY 가 필요한데 goods 는 이미 갖고 있으므로 여기서 프록시.
  //   ⚠️ 원가는 사내 전용 — 이 라우터 자체가 CLAUDE_FEED_KEY fail-closed 가드 뒤에 있음.
  r.get('/inventory-cost-coverage', async (req, res) => {
    const base = (process.env.INVENTORY_API_URL || '').replace(/\/$/, '');
    const key = process.env.INVENTORY_API_KEY || '';
    if (!base || !key) return res.status(503).json({ error: 'inventory_env_missing' });
    const ctrl = new AbortController();
    const to = setTimeout(() => ctrl.abort(), 20000);
    try {
      const resp = await fetch(base + '/api/hooks/cost-coverage', {
        headers: { 'X-API-Key': key }, signal: ctrl.signal
      });
      clearTimeout(to);
      const text = await resp.text();
      let data; try { data = text ? JSON.parse(text) : null; } catch { data = { raw: text }; }
      if (!resp.ok) return res.status(resp.status).json({ error: 'inventory_error', status: resp.status, data });
      res.json(data);
    } catch (e) {
      clearTimeout(to);
      res.status(502).json({ error: 'inventory_unreachable', detail: e.message });
    }
  });

  // ━━━ 🖥️ 시장가 조사 큐 (2026-07-25) — 맥미니 데몬이 pending 픽업 + 결과 반환 ━━━
  //   GET  /market-research         → 처리 대기(pending) 요청 목록 {items:[{cpId,productName,category,size,material,ourTargetKRW,specText,requestedAt}]}
  //   POST /market-research/result  → 데몬이 조사 결과 반환 {cpId, result:{observedSpec,brands,summary,searches?}, error?}
  const mrStore = require('./market-research-store');
  r.get('/market-research', (req, res) => {
    try { res.json({ items: mrStore.listPending() }); }
    catch (e) { res.status(500).json({ error: e.message }); }
  });
  r.post('/market-research/result', (req, res) => {
    try {
      const b = req.body || {};
      const cpId = String(b.cpId || '').trim();
      if (!cpId) return res.status(400).json({ error: 'cpId required' });
      if (b.error) { mrStore.complete(cpId, null, String(b.error).slice(0, 300)); return res.json({ ok: true, status: 'error' }); }
      const rec = mrStore.get(cpId);
      const result = b.result || {};
      // 아이템 정규화 — price 숫자화, 무가격 제거, structMatch 불리언화 (데몬이 문자열로 줄 수 있음)
      const normItems = (arr) => (Array.isArray(arr) ? arr : []).map(it => ({
        name: String(it.name || '').slice(0, 120),
        price: (Number(String(it.price).replace(/[^0-9.]/g, '')) > 0) ? Number(String(it.price).replace(/[^0-9.]/g, '')) : null,
        url: (typeof it.url === 'string' && /^https?:\/\//.test(it.url)) ? it.url : null,
        seller: String(it.seller || '').slice(0, 60),
        structMatch: it.structMatch === true
      })).filter(it => it.price);
      if (Array.isArray(result.brands)) result.brands = result.brands.map(br => ({
        brand: String(br.brand || '').slice(0, 60),
        note: String(br.note || '').slice(0, 200),
        kr: { items: normItems(br.kr && br.kr.items) },
        tw: { items: normItems(br.tw && br.tw.items) },
        hk: { items: normItems(br.hk && br.hk.items) },   // 🇭🇰 홍콩(HKD) 실검색 (2026-07-25)
        th: { items: normItems(br.th && br.th.items) }    // 🇹🇭 태국(THB) 실검색 (2026-07-25)
      }));
      result.observedSpec = String(result.observedSpec || '').slice(0, 300);
      result.summary = String(result.summary || '').slice(0, 500);
      // UI 렌더용 필드 보강 (mode/우리가격/시점) — 평균·갭은 클라이언트가 채택항목으로 재계산
      result.mode = 'search';
      result.imageSource = result.imageSource || 'mac-daemon';
      result.usedImage = !!(result.observedSpec);
      result.ourPriceKRW = (rec && rec.request && Number(rec.request.ourTargetKRW) > 0) ? Number(rec.request.ourTargetKRW) : null;
      result.generatedAt = new Date().toISOString();
      mrStore.complete(cpId, result, null);
      res.json({ ok: true, status: 'done' });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  // ━━━ 🖥️ 견적서 자동 파싱 큐 (2026-07-25) — 맥미니 데몬이 pending 픽업 + 파싱결과 반환 ━━━
  //   GET  /quote-parse        → pending {items:[{fileId,name,mime,recordId,payload:{mode,base64/text,...},requestedAt}]}
  //   POST /quote-parse/result → {fileId, parsed:{vendor,lineItems,...}, error?} → parsedDb placeholder 채움
  const _pathq = require('path');
  const quoteStore = require('./quote-parse-store');
  const inboxW = require('./inbox-watcher');
  const PARSED_DB_PATH_Q = _pathq.join(
    process.env.PARSED_DB_DIR || (process.env.NODE_ENV === 'production' ? '/data' : _pathq.join(__dirname, '..', 'data')),
    'parsed-quotes.json'
  );
  r.get('/quote-parse', (req, res) => {
    try { res.json({ items: quoteStore.listPending() }); }
    catch (e) { res.status(500).json({ error: e.message }); }
  });
  r.post('/quote-parse/result', (req, res) => {
    try {
      const b = req.body || {};
      const fileId = String(b.fileId || '').trim();
      if (!fileId) return res.status(400).json({ error: 'fileId required' });
      const db = inboxW.loadParsedDb(PARSED_DB_PATH_Q);
      const rec = inboxW.finalizeParsedRecord(db, fileId, b.parsed || null, b.error || null);
      inboxW.saveParsedDb(PARSED_DB_PATH_Q, db);
      quoteStore.remove(fileId);
      res.json({ ok: true, finalized: !!rec });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  // ━━━ 🖥️ 가격 일괄 점검 큐 (2026-07-25) — 맥미니 데몬이 청크 픽업 + 7개국가 추정 반환 ━━━
  //   GET  /pricing-audit        → pending 청크 {items:[{jobId,chunkIndex,products:[{id,name,category,krwPrice,size,material,origin}]}]}
  //   POST /pricing-audit/result → {jobId, chunkIndex, prices:{productId:{KR,TW,HK,CN,TH,US,JP}}}
  const paStore = require('./pricing-audit-store');
  r.get('/pricing-audit', (req, res) => {
    try { res.json({ items: paStore.listPending() }); }
    catch (e) { res.status(500).json({ error: e.message }); }
  });
  r.post('/pricing-audit/result', (req, res) => {
    try {
      const b = req.body || {};
      const jobId = String(b.jobId || '').trim();
      if (!jobId) return res.status(400).json({ error: 'jobId required' });
      const job = paStore.completeChunk(jobId, Number(b.chunkIndex), b.prices || {});
      res.json({ ok: true, remaining: job ? job.remainingChunks : null });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  return r;
}

module.exports = { router };
