    ...
    for (const it of items) {
      if (it.mimeType === 'application/vnd.google-apps.folder') {
        if (recursive && depth < maxDepth) {
          folderPath.set(it.id, path + '/' + it.name);
          await listOne(it.id, path + '/' + it.name, depth + 1);
        }
        continue;
      }
...
async function importDriveFolder({ folderId, fileIds, dryRun, parsedDbPath, anthropicKey, userCountry, recursive }) {
  if (!folderId) throw new Error('folderId 필요');
  if (!anthropicKey) throw new Error('anthropicKey 필요');
  if (!parsedDbPath) throw new Error('parsedDbPath 필요');
