...
// 비제품(운임·포장·일회성) 이름 패턴 — 품명 오염 방지 (2026-06-15)
const NON_PRODUCT_NAME_RE = /freight|shipping|운임|운송|배송|express\s*(delivery|service)|courier|sea\s*freight|air\s*freight|\bfob\b|\bcif\b|\bdap\b|\bddp\b|paper\s*box|carton|packing|packaging|포장|opp\s*bag|poly\s*bag|hang\s*tag|mold\s*fee|sample\s*fee|setup\s*fee|금형비|샘플비/i;
function buildRecord(file, parsed, userCountry) {
  const lineItems = Array.isArray(parsed.lineItems) ? parsed.lineItems : [];
  // type 이 product 여도 이름이 운임/포장이면 제품에서 제외 → 품명 데이터 오염 차단
  const isProductLine = li => (li.type === 'product' || (!li.type && li.unitPrice && li.qty)) && !NON_PRODUCT_NAME_RE.test(li.name || '');
  const products = lineItems.filter(isProductLine);
  const packaging = lineItems.filter(li => li.type === 'packaging' || (!isProductLine(li) && /box|carton|packing|포장|bag/i.test(li.name||'')));
  const oneTime = lineItems.filter(li => li.type === 'one_time');
  const other = lineItems.filter(li => li.type === 'other' || (li.type !== 'product' && li.type !== 'packaging' && li.type !== 'one_time' && !isProductLine(li)));
