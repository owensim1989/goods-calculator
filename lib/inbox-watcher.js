...
// 원본 파일 스트림 (2026-08-20 신설) — 견적 카드에서 원본 다운로드·미리보기
// buffer 로 전부 읽지 않고 스트림으로 넘겨 메모리 사용을 일정하게 유지
async function openDriveFileStream(fileId) {
...
// 원본 생존 확인 (2026-08-20 신설) — 헬스체크용. 휴지통도 유실로 취급.
async function probeDriveFile(fileId) {
  const drive = await getDriveClient();
  const res = await drive.files.get({ fileId, fields: 'id, name, trashed', ...SUPPORTS_ALL });
  const f = res.data || {};
  if (f.trashed) { const e = new Error('휴지통에 있음'); e.code = 404; throw e; }
  return f;
}

// 파일명으로 재탐색 (2026-08-20 신설)
// 사람이 Drive 에서 파일을 옮기거나 복사본으로 바꾸면 driveFile.id 가 죽는다.
// import 로직의 id→name 2단 매칭과 같은 안전망 — 다운로드 실패 시 1회 복구 시도용.
...
const PARSE_PROMPT = [
  ...
  '',
  '규칙:',
  '- quoteDate: 견적서에 표기된 발행일/작성일/Date/Quotation Date/报价日期/见积日 를 YYYY-MM-DD 로. 표기가 전혀 없으면 null (파일명·임의 추정 금지)',
...
function buildRecord(file, parsed, userCountry) {
  ...
  return {
    ...
    // 견적서에 표기된 발행일 (없으면 null → push 시 Drive 업로드일로 fallback)
    quoteDate: (parsed.quoteDate && /^\d{4}-\d{2}-\d{2}$/.test(parsed.quoteDate)) ? parsed.quoteDate : null,
...
function parsedToCacheItems(parsedDb) {
  ...
  for (const it of items) {
    ...
    const 수요처유형 = overrides.수요처유형 || null;  // 자체브랜드/고객사/라이선스·콜라보
    const 수요처명 = overrides.수요처명 || null;
    const ovProducts = Array.isArray(overrides.products) ? overrides.products : [];
    // 견적일자: 사용자 override > 문서 표기 발행일 > Drive 업로드일(fallback) — AI 견적은 절대 날짜 비우지 않음
    const _driveDate = (it.driveFile && it.driveFile.uploadedAt) ? String(it.driveFile.uploadedAt).slice(0, 10) : null;
    const 견적일자 = overrides.견적일자 || it.quoteDate || _driveDate || null;
    ...
    merged.forEach((p, idx) => {
      ...
      out.push({
        ...
        _isAiParsed: true,
        // 통합DB 행에서 원본 📎 아이콘 노출 여부 (2026-08-20) — Drive 파일 또는 메일 원문
        _hasOriginal: !!((it.driveFile && it.driveFile.id) || it.rawText)
      });
    });
  }
  return out;
...
// ─────────────────────────────────────────────────────────────
// 🖥️ 2026-07-25: 정액제 전환 — 스캔·파일이동은 goods, AI 파싱만 맥미니 데몬 큐로 위임
// ─────────────────────────────────────────────────────────────
function _fileToParsePayload(buf, mimeType) {
  const kind = SUPPORTED_MIMES[mimeType];
  if (!kind) throw new Error('지원 안 함: ' + mimeType);
  if (kind === 'excel') {
    let XLSX; try { XLSX = require('xlsx'); } catch (e) { throw new Error('xlsx 모듈 미설치'); }
    const wb = XLSX.read(buf, { type: 'buffer' });
    const parts = [];
    wb.SheetNames.slice(0, 3).forEach(n => { const csv = XLSX.utils.sheet_to_csv(wb.Sheets[n], { blankrows: false }); parts.push('[Sheet: ' + n + ']\n' + csv.slice(0, 6000)); });
    return { mode: 'text', kind: 'excel', text: parts.join('\n\n---\n\n') };
  }
  return { mode: 'base64', kind, mime: mimeType, base64: buf.toString('base64') };
}
function _placeholderRecord(file) {
  const ownerEmail = file.owners && file.owners[0] && file.owners[0].emailAddress;
  const ownerName = file.owners && file.owners[0] && file.owners[0].displayName;
  return {
    id: newId(), createdAt: new Date().toISOString(), source: 'drive-inbox',
    driveFile: { id: file.id, name: file.name, mimeType: file.mimeType, size: file.size, ownerEmail: ownerEmail || null, ownerName: ownerName || null, webViewLink: file.webViewLink || null, uploadedAt: file.createdTime || null, movedTo: null },
    vendor: null, quoteDate: null, country: null, countryOfOrigin: null, countrySource: null,
    currency: 'USD', surchargeEstimateKRW: null, raw: null,
    products: [], packaging: [], oneTime: [], other: [],
    reviewStatus: 'parsing', reviewedBy: null, reviewedAt: null, rejectReason: null, manualOverrides: null
  };
}
// 스캔 → 각 신규 파일: 페이로드 캡처 → enqueueFn 로 데몬 큐 적재 → placeholder 기록 → _processed 이동 (AI 호출 없음)
async function runOnceQueued(opts) {
  const { folderId, parsedDbPath, enqueueFn } = opts;
  if (!folderId) throw new Error('folderId 필요');
  if (!parsedDbPath) throw new Error('parsedDbPath 필요');
  if (typeof enqueueFn !== 'function') throw new Error('enqueueFn 필요');
  const drive = await getDriveClient();
  const result = { startedAt: new Date().toISOString(), scanned: 0, queued: 0, failed: 0, skipped: 0, errors: [] };
  const failedId = await ensureSubfolder(drive, folderId, '_failed');
  const processedRootId = await ensureSubfolder(drive, folderId, '_processed');
  const yyyymm = (() => { const d = new Date(Date.now() + 9 * 3600 * 1000); return d.getUTCFullYear() + '-' + String(d.getUTCMonth() + 1).padStart(2, '0'); })();
  const monthId = await ensureSubfolder(drive, processedRootId, yyyymm);
  const files = await listInboxFiles(drive, folderId);
  result.scanned = files.length;
  const parsedDb = loadParsedDb(parsedDbPath);
  for (const file of files) {
    try {
      if (parsedDb.items.find(it => it.driveFile && it.driveFile.id === file.id)) { result.skipped++; continue; }
      if (!SUPPORTED_MIMES[file.mimeType]) { result.skipped++; continue; }
      const buf = await downloadFileBuffer(drive, file.id);
      const payload = _fileToParsePayload(buf, file.mimeType);
      const rec = _placeholderRecord(file);
      await enqueueFn({ fileId: file.id, name: file.name, mime: file.mimeType, recordId: rec.id, payload });
      parsedDb.items.push(rec);
      const oldParents = (file.parents || [folderId]).join(',');
      await moveFile(drive, file.id, monthId, oldParents);
      rec.driveFile.movedTo = '_processed/' + yyyymm + '/';
      result.queued++;
    } catch (err) {
      result.failed++;
      const msg = (err && err.message) || String(err);
      result.errors.push({ file: file.name, error: msg });
      try { const op = (file.parents || [folderId]).join(','); await uploadErrorSidecar(drive, failedId, file.name, err.stack || msg); await moveFile(drive, file.id, failedId, op); } catch (e) {}
    }
  }
  parsedDb.lastRun = new Date().toISOString();
  saveParsedDb(parsedDbPath, parsedDb);
...
// 데몬 파싱 결과 수신 → placeholder(reviewStatus:'parsing') 를 실제 데이터로 채움 (buildRecord 매핑 재사용)
function finalizeParsedRecord(parsedDb, fileId, parsed, error) {
  const it = (parsedDb.items || []).find(x => x.driveFile && x.driveFile.id === fileId && x.reviewStatus === 'parsing');
  if (!it) return null;
  if (error || !parsed) { it.reviewStatus = 'pending'; it.parseError = String(error || 'no_result').slice(0, 300); return it; }
  const lineItems = Array.isArray(parsed.lineItems) ? parsed.lineItems : [];
  it.products = lineItems.filter(li => li.type === 'product' || (!li.type && li.unitPrice && li.qty));
  it.packaging = lineItems.filter(li => li.type === 'packaging');
  it.oneTime = lineItems.filter(li => li.type === 'one_time');
  it.other = lineItems.filter(li => li.type === 'other');
  it.currency = mode(lineItems.map(li => li.currency).filter(Boolean)) || 'USD';
  it.vendor = parsed.vendor || null;
  it.quoteDate = (parsed.quoteDate && /^\d{4}-\d{2}-\d{2}$/.test(parsed.quoteDate)) ? parsed.quoteDate : null;
  it.countryOfOrigin = parsed.countryOfOrigin || null;
  it.country = COUNTRY_LABEL[parsed.countryOfOrigin] || null;
  it.countrySource = parsed.countryOfOrigin ? 'ai' : null;
  it.surchargeEstimateKRW = parsed.surchargeEstimate_KRW || null;
  it.raw = parsed;
...
// ✍️직접입력·📧메일 건은 원본 파일이 없다. 나중에 실제 견적서를 붙일 수 있게 한다.
// ⚠️ AI 재파싱은 절대 하지 않는다 — 이미 검수된 숫자를 파일 내용으로 덮어쓰면 안 된다.
//    파일만 붙이고 파싱 결과(products·vendor·country)는 그대로 둔다.
// ─────────────────────────────────────────────────────────────
async function attachFileToRecord(opts) {
  const { folderId, parsedDbPath, recordId, filename, mimeType, buffer } = opts;
  const submitterName = opts.submitterName || null;
  const submitterEmail = opts.submitterEmail || null;
  if (!folderId) throw new Error('folderId 필요 (INBOX_DRIVE_FOLDER_ID 미설정)');
  if (!parsedDbPath) throw new Error('parsedDbPath 필요');
  ...
  const parsedDb = loadParsedDb(parsedDbPath);
  const rec = (parsedDb.items || []).find(x => x.id === recordId);
  if (!rec) throw new Error('견적 레코드를 찾을 수 없습니다: ' + recordId);
  if (rec.driveFile && rec.driveFile.id) throw new Error('이미 원본 파일이 붙어있는 견적입니다 (덮어쓰기 방지)');
  ...
  // ⚠️ 인박스 루트(folderId)에 올렸다가 옮기면 안 된다 — 감시자가 루트만 스캔하므로
  //    이동 실패하거나 그 찰나에 스캔이 돌면 첨부 파일이 "새 견적"으로 다시 파싱돼
  //    중복 레코드가 생긴다. AI 파싱이 필요 없는 첨부는 처음부터 _processed/ 에 넣는다.
  const { Readable } = require('stream');
  const created = await drive.files.create({
    requestBody: {
      name: filename,
      parents: [monthId],
      description: '사후 첨부 (' + recordId + ')' +
        (submitterName || submitterEmail ? ' · ' + [submitterName, submitterEmail].filter(Boolean).join(' / ') : '')
    },
    media: { mimeType, body: Readable.from(buffer) },
  ...
  const prev = rec.driveFile || {};

  // ⚠️ 파생 필드 고정 — 캐시행 프로젝트명은 driveFile.name 에서 유도된다(parsedToCacheItems).
  //    파일명이 바뀌면 통합DB·노션 행 이름이 따라 바뀌므로, 지금 값을 overrides 에 못 박는다.
  rec.manualOverrides = rec.manualOverrides || {};
  if (!rec.manualOverrides.프로젝트명) {
    const vendorNow = rec.manualOverrides.거래처 || rec.vendor || '미상';
    rec.manualOverrides.프로젝트명 = '[AI] ' + (prev.name || vendorNow);
  }

  rec.driveFile = Object.assign({}, prev, {
    id: file.id,
    name: file.name,
    mimeType: file.mimeType,
    size: file.size,
    webViewLink: file.webViewLink || null,
    // ⚠️ uploadedAt 은 건드리지 않는다 — 견적일자 fallback 이 여기서 나온다.
    //    오늘 날짜를 넣으면 7월에 받은 견적이 8월 견적으로 둔갑한다.
    uploadedAt: prev.uploadedAt || null,
    movedTo: '_processed/' + yyyymm + '/',
    attachedLater: true,
    attachedAt: new Date().toISOString(),
    attachedBy: submitterEmail || submitterName || null,
    origName: prev.name || null
  });

  saveParsedDb(parsedDbPath, parsedDb);
  return { record: rec, file };
}

// ─────────────────────────────────────────────────────────────
// 텍스트(메일 본문·메시지 등) 직접 AI 파싱 (2026-05-29 신설)
// Drive 업로드 없이 callClaude 로 견적 정보 추출 → parsedDb 미검수 추가
// 파일 업로드와 동일한 PARSE_PROMPT·buildRecord 재사용 (검수·통합DB 흐름 일치)
// ─────────────────────────────────────────────────────────────
async function parseTextWithClaude(text, anthropicKey) {
  const content = [{
    type: 'text',
    text: PARSE_PROMPT + '\n\n아래는 견적서 또는 견적 관련 메일/메시지 본문입니다:\n\n' + String(text).slice(0, 30000)
  }];
  const out = await callClaude([{ role: 'user', content }], anthropicKey, { max_tokens: 3000 });
  const parsed = extractJSON(out);
  if (!parsed) {
...
async function processOneText(opts) {
  const { parsedDbPath, anthropicKey } = opts;
  if (!anthropicKey) throw new Error('anthropicKey 필요 (ANTHROPIC_API_KEY 미설정)');
  if (!parsedDbPath) throw new Error('parsedDbPath 필요');
  const clean = String(opts.text || '').trim();
  if (!clean) throw new Error('텍스트 내용 비어있음');

  const parsed = await parseTextWithClaude(clean, anthropicKey);

  // Drive 파일 없는 가상 file 객체 (제출자·제목 hint 만)
  const titleHint = clean.replace(/\s+/g, ' ').slice(0, 40) || '메일/텍스트';
  const file = {
    id: null,
    name: '📧 ' + titleHint,
    mimeType: 'text/plain',
    size: clean.length,
    owners: (opts.submitterEmail || opts.submitterName)
      ? [{ emailAddress: opts.submitterEmail || null, displayName: opts.submitterName || null }]
      : [],
    webViewLink: null,
  ...
  const record = buildRecord(file, parsed, opts.userCountry || null);
  record.source = 'page-text';
  record.driveFile.movedTo = null;
  record.rawText = clean.slice(0, 20000); // 원문 보존 (검수 참고용)
...
// ─────────────────────────────────────────────────────────────
// 받은 견적 직접(수동) 입력 (2026-05-29 신설)
// AI 파싱 없이 사용자가 입력한 거래처·국가·제품을 미검수 record 로 등록
// 파일·텍스트 업로드와 동일한 buildRecord·검수·통합DB 흐름 재사용
// ─────────────────────────────────────────────────────────────
async function processOneManual(opts) {
  const { parsedDbPath } = opts;
  if (!parsedDbPath) throw new Error('parsedDbPath 필요');
  ...
  products.forEach(p => {
    ...
    if (!name && !unitPrice && !qty) return; // 빈 행 skip
    lineItems.push({
      type: 'product',
      name: name || '(미입력)',
      qty,
      unitPrice,
      currency: p.currency || opts.currency || 'KRW',
      totalAmount: (p.totalAmount != null && p.totalAmount !== '') ? Number(p.totalAmount) : qty * unitPrice,
      material: (p.spec ? String(p.spec).trim() : null) || null,
      size: null,
      notes: null
    });
  });
  if (!lineItems.length) throw new Error('제품을 최소 1개 입력해주세요');

  const parsed = { vendor: opts.vendor || null, countryOfOrigin: null, lineItems };
  const file = {
    id: null,
    name: '✍️ ' + (opts.vendor ? opts.vendor + ' 견적' : '수동 입력 견적'),
    mimeType: 'text/plain',
    size: 0,
    owners: (opts.submitterEmail || opts.submitterName)
      ? [{ emailAddress: opts.submitterEmail || null, displayName: opts.submitterName || null }]
  ...
  record.driveFile.movedTo = null;
  if (opts.memo) record.memo = String(opts.memo).slice(0, 2000);
  // 수취 법인 소재국 (한국 본사 / 대만 지사 등) — 견적 발급국(country)과 별개
  if (opts.receivingEntity) record.receivingEntity = String(opts.receivingEntity).slice(0, 40);
