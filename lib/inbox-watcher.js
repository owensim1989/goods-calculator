...
async function listAnyFolderFiles(drive, folderId) {
  // 폴더 안 모든 파일 (서브폴더 제외). SUPPORTED_MIMES 필터링은 caller 에서
  const q = "'" + folderId + "' in parents and trashed = false and mimeType != 'application/vnd.google-apps.folder'";
  const res = await drive.files.list({
    q,
  ...
  return (res.data.files || []).filter(f => !f.name.endsWith('.error.txt'));
}

async function previewDriveFolder({ folderId, parsedDb }) {
  if (!folderId) throw new Error('folderId 필요');
  const drive = await getDriveClient();
  const files = await listAnyFolderFiles(drive, folderId);
...
async function importDriveFolder({ folderId, fileIds, dryRun, parsedDbPath, anthropicKey, userCountry }) {
  if (!folderId) throw new Error('folderId 필요');
  if (!anthropicKey) throw new Error('anthropicKey 필요');
  if (!parsedDbPath) throw new Error('parsedDbPath 필요');
  ...
  const parsedDb = loadParsedDb(parsedDbPath);
  const preview = await previewDriveFolder({ folderId, parsedDb });
