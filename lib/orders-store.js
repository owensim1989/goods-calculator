...
function createOrder(partial) {
  ...
  const order = {
    ...
    manual_rate: false,    // 사용자가 환율 input 을 수동 입력하면 true. 통화 변경 시 자동 prefill 안 함
    rate_locked_at: null,
    rate_source: null,
    validity_until: null,
    summary_mode: false,
...
function updateOrder(orderId, patch, who) {
  ...
  const ALLOWED_FIELDS = [
    ...
    'rate', 'manual_rate', 'rate_locked_at', 'rate_source', 'validity_until',
  ...
  // 환율 류는 양수만 허용 — 음수·0·NaN 이면 무시 (기존 값 유지). 음수 환율 → 인보이스 금액 음수 방지
  for (const rk of ['rate', 'manual_rate']) {
    if (patch[rk] !== undefined && !(Number(patch[rk]) > 0)) delete patch[rk];
  }
...
function computeTotals(order) {
  const rate = (Number(order.rate) > 0) ? Number(order.rate) : 1;  // 음수·0·NaN 방어
  let totalKrw = 0;
  let totalQty = 0;
