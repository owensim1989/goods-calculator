...
function parseBuyerExcel(buffer, XLSX) {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    items.push({
      ...
      discount_rate:colDisc >= 0     ? (parseFloat(String(r[colDisc] || '0').replace(/,/g, '')) || 0) : 0,
      order_qty:    qty,
      ...
      unit_usd_src: colUnitUsd >= 0  ? (parseFloat(String(r[colUnitUsd] || '0').replace(/,/g, '')) || 0) : 0,
      sub_usd_src:  colSubUsd >= 0   ? (parseFloat(String(r[colSubUsd] || '0').replace(/,/g, '')) || 0) : 0
    });
  }
...
<div class="preview">
  ...
  <div class="terms">
    ...
    <p style="margin-top:12px">Please remit the full amount of <strong>USD $${_fmtUsd(totalUsd)}</strong>${dueDate ? ' by ' + dueDate : ''}. Bank charges (sender and intermediary) to be borne by buyer. Upon confirmation of payment, goods will be prepared and shipped within 5-7 business days from our warehouse in Korea.</p>
  </div>
  <div class="signature">
...
// 회사 정보 single source of truth = business/contracts/company-master.json
// 2026-05-22 placeholder 값 정정: 부산 가짜 주소·sales@jeisha 가짜 이메일·Shinhan 가짜 계좌 → 진짜 값
const DEFAULT_JEISHA = {
  name: 'JEISHA Co., Ltd.',
  bizno: '503-86-14953',
  address: 'Dalgubeol-daero 2221, Jung-gu, Daegu, Republic of Korea (41948)',
  tel: '',
  email: 'info@jeisha.kr',
  rep: 'Owen Sim (CEO)'
};
const DEFAULT_BANK = {
  bene: 'JEISHA Co., Ltd.',
  ...
  swift: 'CZNBKRSE',
  account: '612968-11-002904',
  addr: '26, Gukjaegeumyung-Ro-8-Gil, Yeongdeungpo-Gu, Seoul, South Korea (07331)'
};
...
function router(deps) {
  ...
  // POST /:id/cancel — 수출 주문 취소 (2026-06-01)
  // 출고 전(미출고·미입금) + 인보이스 발행됨 → business 인보이스 자동 취소 후 주문 삭제
  // shipped/paid 는 자동 취소 거부(409) — inventory 재고·환불 회계는 수동 정리
  r.post('/:id/cancel', async (req, res) => {
    ...
    if (status === 'shipped' || status === 'paid') {
      return res.status(409).json({
        error: 'manual_required', status, invoice_no: o.invoice_no || null,
        message: status === 'shipped'
          ? '출고된 주문 — inventory 재고를 수동으로 복원해야 합니다'
          : '입금된 주문 — business 인보이스는 credit note 로 수동 처리해야 합니다'
      });
    }
    let invoiceCancelled = false, invoiceResult = null;
    // 견적 발급된 주문(quote_no 보유) + 미출고·미입금 → business 인보이스 자동 취소 시도
    // invoice_no 가 아닌 quote_no 기준: 발행 중 timeout 등으로 goods 가 invoice_no 를 못 받은
    // orphan 인보이스도 quote_no 로 찾아 정리 (business 가 not_found 면 무해)
    if (o.quote_no) {
      const businessUrl = process.env.BUSINESS_API_URL || '';
      const apiKey = process.env.GOODS_TO_BUSINESS_API_KEY || process.env.BUSINESS_API_KEY || '';
      if (!businessUrl || !apiKey) {
      ...
      try {
        const ctrl = new AbortController();
        const to = setTimeout(() => ctrl.abort(), 15000);
        const resp = await fetch(businessUrl.replace(/\/$/, '') + '/api/hooks/cancel-invoice-from-goods', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-API-Key': apiKey },
          body: JSON.stringify({
            quote_no: o.quote_no,
            goods_order_id: o.order_id,
            reason: (req.body && req.body.reason) || '수출 주문 취소'
          }),
          signal: ctrl.signal
        });
        clearTimeout(to);
        const data = await resp.json().catch(() => ({}));
        if (resp.status === 409) {
          // business 측에서 입금완료 판정 (race) — 자동 취소 불가
          return res.status(409).json({ error: 'invoice_paid', message: data.message || '입금완료 인보이스 — 수동 처리 필요', invoice_no: o.invoice_no });
        }
        if (!resp.ok) {
          return res.status(resp.status).json({ error: 'business invoice cancel failed', detail: data });
        }
        invoiceCancelled = !data.not_found;
        invoiceResult = data;
      } catch (e) {
        console.error('[orders/cancel] business webhook 실패:', e);
        return res.status(500).json({ error: 'business webhook error: ' + e.message });
      }
    }
    // 인보이스 취소 성공(또는 인보이스 없음) → 주문 삭제
    store.deleteOrder(o.order_id);
    res.json({ ok: true, cancelled: true, invoiceCancelled, invoice_no: o.invoice_no || null, invoice: invoiceResult });
  });
