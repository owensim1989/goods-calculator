...
function createProject(partial = {}) {
  ...
  const p = {
    ...
    client_ref: (partial.client_ref && partial.client_ref.id != null)     // business 거래처에서 고른 경우 {id,name}
      ? { id: String(partial.client_ref.id), name: partial.client_ref.name || String(partial.client || '') } : null,
  ...
  saveDb(db);
  // 발주처 자동완성 이력 누적 (실패해도 프로젝트 생성은 성공)
  if (p.client) { try { touchClient(p.client, { ref: p.client_ref, who: partial.who }); } catch (e) {} }
  return p;
}
...
const ALLOWED_FIELDS = [
  'name', 'emoji', 'status', 'brand', 'client', 'client_ref', 'consumerPricingId', 'target', 'barcode', 'design', 'vendors',
  'samples', 'po', 'payments', 'inbound', 'attachments', 'ref_links', 'links', 'checklist', 'notes', 'stages',
  'stage_owners'   // 단계별 담당 오버라이드 { stage: '디자인팀' } — 기본값은 프론트 STAGE_TEAM
];
...
function updateProject(id, patch = {}, who) {
  ...
  db.projects[idx] = p;
  saveDb(db);
  // 발주처가 새로 들어오거나 바뀌면 자동완성 이력 누적
  if (patch.client !== undefined && String(patch.client || '').trim()) {
    try { touchClient(patch.client, { ref: patch.client_ref !== undefined ? patch.client_ref : p.client_ref, who }); } catch (e) {}
  }
  return p;
}
...
// ━━━ 🤝 발주처(외주 고객사) 사용 이력 registry ━━━ (2026-08-13)
// 새 프로젝트 '발주처' 입력 자동완성용 — 쓸 때마다 쌓인다(uses/last_used_at).
// business 거래처 마스터가 원본(authoritative)이고 이 파일은 "이 파이프라인에서 실제로 쓴 발주처" 이력.
// business 에 없는 발주처(신규 고객사)도 여기 남아 다음 입력부터 자동완성됨.
const CLIENTS_FILE = path.join(DATA_DIR, 'pipeline-clients.json');

// 사명 정규화 — business hooks/partner-match normCompanyName 과 동일 규칙(중복 판정 일치)
...
function listClients() {
  return _loadJson(CLIENTS_FILE, 'clients').clients
    .sort((a, b) => (b.uses || 0) - (a.uses || 0) || String(b.last_used_at || '').localeCompare(String(a.last_used_at || '')));
}

// 발주처 사용 기록 (멱등 upsert, 정규화명 기준). ref = business 거래처 {id,name} (선택)
function touchClient(name, opts = {}) {
  const nm = String(name || '').trim().slice(0, 120);
  if (!nm) return null;
  const key = normClientName(nm);
  if (!key) return null;
  const db = _loadJson(CLIENTS_FILE, 'clients');
  let rec = db.clients.find(c => c.key === key);
  if (!rec) {
    rec = { key, name: nm, ref: null, uses: 0, first_used_at: _now(), last_used_at: _now(), by: opts.who || 'goods' };
    db.clients.push(rec);
  }
  rec.name = nm;                                     // 최근 표기를 대표 표기로
  if (opts.ref && opts.ref.id != null) rec.ref = { id: String(opts.ref.id), name: opts.ref.name || nm };
  if (opts.countOnly !== true) rec.uses = (rec.uses || 0) + 1;
  rec.last_used_at = _now();
  if (opts.who) rec.by = opts.who;
  _saveJson(CLIENTS_FILE, db);
  return rec;
}

// 기존 프로젝트들의 client 값을 registry 로 백필 (최초 1회·멱등)
function syncClientsFromProjects() {
  const db = _loadJson(CLIENTS_FILE, 'clients');
  const seen = new Map(db.clients.map(c => [c.key, c]));
  let added = 0;
  for (const p of loadDb().projects) {
    const nm = String(p.client || '').trim();
    if (!nm) continue;
    const key = normClientName(nm);
    if (!key) continue;
    const ref = (p.client_ref && p.client_ref.id != null) ? { id: String(p.client_ref.id), name: p.client_ref.name || nm } : null;
    const exist = seen.get(key);
    if (exist) {
      if (!exist.ref && ref) exist.ref = ref;
      continue;
    }
    const rec = { key, name: nm, ref, uses: 1, first_used_at: p.created_at || _now(), last_used_at: p.updated_at || _now(), by: 'backfill' };
    db.clients.push(rec); seen.set(key, rec); added++;
  }
  if (added) _saveJson(CLIENTS_FILE, db);
  return { added, total: db.clients.length };
}

// ── MyDesk 결제요청 완료 → 대금 기록 (2026-08-18) ──
// payments 는 기록 전용 원칙 유지 — mydesk 완료건을 paid_at 확정 상태로 append.
// 멱등: 같은 mydesk_request_id 가 이미 있으면 기존 항목 반환 (재시도·중복 webhook 대비)
function recordExpensePayment(id, entry = {}, who) {
  const db = loadDb();
  const idx = db.projects.findIndex(p => p.id === id);
  if (idx < 0) return null;
  const p = db.projects[idx];
  if (!Array.isArray(p.payments)) p.payments = [];
  const reqId = String(entry.mydesk_request_id || '').trim();
  if (reqId) {
    const dup = p.payments.find(x => x.mydesk_request_id === reqId);
    if (dup) return { project: p, payment: dup, idempotent: true };
  }
  const pay = {
    kind: ['deposit', 'balance', 'sample', 'shipping', 'etc'].includes(entry.kind) ? entry.kind : 'etc',
    amount: Number(entry.amount) || 0,
    cur: String(entry.cur || 'KRW').slice(0, 8),
    due: null,
    paid_at: (entry.paid_at || _now()).slice(0, 10),
    memo: String(entry.memo || '').slice(0, 300),
    mydesk_request_id: reqId || undefined,
    source: 'mydesk'
  };
  p.payments.push(pay);
  p.history.push({ at: _now(), ev: 'note', detail: `💸 MyDesk 결제요청 완료 → 대금 기록: ${pay.amount.toLocaleString()} ${pay.cur}${pay.memo ? ' · ' + pay.memo : ''}`, by: who || 'mydesk' });
  p.updated_at = _now();
  db.projects[idx] = p;
  saveDb(db);
  return { project: p, payment: pay, idempotent: false };
}
...
module.exports = {
  ...
  CLIENTS_FILE, normClientName, listClients, touchClient, syncClientsFromProjects,
