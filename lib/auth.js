...
// Railway 영구 볼륨 사용 시 SESSIONS_FILE env 로 /data/goods-sessions.json 같이 지정
//   → 옛 default `__dirname/../data/...` 는 Railway 의 /app/data (재배포 시 휘발) 라 매 deploy 마다 로그아웃 됨
const SESSIONS_FILE = process.env.SESSIONS_FILE || path.join(__dirname, '..', 'data', 'goods-sessions.json');
let sessions = {}; // sid -> { user, createdAt, lastSeenAt }

// 자동 로그인용 remember-me 토큰 store (30일)
// Railway 영구 볼륨 사용 시 REMEMBER_FILE env 로 /data/goods-remember.json 지정 권장
const rememberStore = createRememberStore({
  file: process.env.REMEMBER_FILE || path.join(__dirname, '..', 'data', 'goods-remember.json'),
  ttlDays: 30
});
