...
const security = require('./security'); // 2026-08-18 보안: 상수시간 키 비교
const constructionStore = require('./construction-store'); // 2026-08-21 시공 발주 링크
...
function router(deps = {}) {
  ...
  r.get('/alerts', (req, res) => {
    ...
    if (!security.keyEq(req.headers['x-api-key'], expected)) return res.status(401).json({ error: 'invalid api key' }); // 2026-08-18 보안: 상수시간
  ...
  // ── MyDesk 결제요청 연동 ① 발주 건 목록 (2026-08-18) — X-API-Key=MYDESK_TO_GOODS_API_KEY ──
  // 결제요청 모달의 "🏭 제품제작" 서브 드롭다운용. 진행중 파이프라인 제품 + 미지급 대금 요약.
  // auth.js isPublicPath 에 /api/pipeline/expense-options 등록, 키 검사는 여기서 (alerts 패턴)
  r.get('/expense-options', (req, res) => {
    const expected = process.env.MYDESK_TO_GOODS_API_KEY || '';
    if (!expected) return res.status(503).json({ error: 'not configured (MYDESK_TO_GOODS_API_KEY)' });
    if (!security.keyEq(req.headers['x-api-key'], expected)) return res.status(401).json({ error: 'invalid api key' });
    const items = store.listProjects({ status: 'active' }).map(p => ({
      id: p.id,
      name: p.name,
      emoji: p.emoji || '',
      type: p.type,                                     // new | reorder
      brand: p.brand || 'own',                          // own=자체 / oem=외주
      client: p.client || '',
      stage: p.stage,
      stageLabel: (store.STAGE_META[p.stage] || {}).label || p.stage,
      vendor: (p.po && p.po.vendor) || '',
      // 정산 프로젝트 링크 (8/17 B안) — 있으면 mydesk 가 이 프로젝트로 귀속 예고
      settlementProject: (p.links && p.links.mydesk_project) ? { id: p.links.mydesk_project.id, name: p.links.mydesk_project.name } : null,
      // 미지급 대금 요약 — mydesk 가 금액·용도 프리필에 사용
      unpaid: (p.payments || []).filter(x => !x.paid_at).map(x => ({
        kind: x.kind, amount: Number(x.amount) || 0, cur: x.cur || 'KRW', due: x.due || null
      }))
    }));
    res.json({ items, url: 'https://goods.jeisha.kr/pipeline.html' });
  });

  // ── MyDesk 결제요청 연동 ② 대금 기록 push (2026-08-18) — 결제요청 완료 시 호출 ──
  // 멱등(mydesk_request_id) — 재시도에도 payments 가 중복 적립되지 않음.
  // auth.js isPublicPath 에 /api/pipeline/:id/expense-record 등록, 키 검사는 여기서
  r.post('/:id/expense-record', (req, res) => {
    const expected = process.env.MYDESK_TO_GOODS_API_KEY || '';
    if (!expected) return res.status(503).json({ error: 'not configured (MYDESK_TO_GOODS_API_KEY)' });
    if (!security.keyEq(req.headers['x-api-key'], expected)) return res.status(401).json({ error: 'invalid api key' });
    const { amount, cur, kind, memo, paid_at, mydesk_request_id, by } = req.body || {};
    if (!(Number(amount) > 0)) return res.status(400).json({ error: 'amount 필요 (양수)' });
    if (!String(mydesk_request_id || '').trim()) return res.status(400).json({ error: 'mydesk_request_id 필요 (멱등키)' });
    const out = store.recordExpensePayment(req.params.id, { amount, cur, kind, memo, paid_at, mydesk_request_id }, by || 'mydesk');
    if (!out) return res.status(404).json({ error: '파이프라인 제품을 찾을 수 없습니다: ' + req.params.id });
    res.json({ ok: true, action: out.idempotent ? 'idempotent_skip' : 'recorded', payment: out.payment, project: { id: out.project.id, name: out.project.name } });
  });
  ...
  r.get('/hooks/business-links', (req, res) => {
    ...
    const key = req.headers['x-api-key'] || ''; // 2026-08-18 보안: 쿼리스트링 키 제거(로그 노출 방지) — 헤더 전용
    // 2026-08-21 진단: 401 이 "goods 에 키가 아예 없음" 인지 "호출자 키가 다름" 인지 구분.
    //   (키 값은 절대 노출하지 않고 원인만 알려준다 — 배지 연동이 조용히 죽어 있던 걸 못 잡아낸 이유)
    if (!allowed.length) return res.status(401).json({ error: 'unauthorized', reason: 'goods_no_key_configured' });
    if (!allowed.some(k => security.keyEq(key, k))) {
      return res.status(401).json({ error: 'unauthorized', reason: key ? 'key_mismatch' : 'caller_sent_no_key' });
    }
    ...
    // 2026-08-21 — 시공 거래처도 함께 (제품 파이프라인을 타지 않아 영원히 '견적만' 이던 문제)
    try {
      links.push(...constructionStore.businessLinks());
    } catch (e) {
      console.warn('[business-links] 시공 링크 실패:', e.message);
    }
    res.json({ links });
  });
  ...
  // ── 🏷️ 자체브랜드(MDN) 정산 프로젝트 생성·연결 (2026-08-17, Owen 확정 = 제품별 프로젝트) ──
  //  왜: 자체브랜드 발주는 대응 고객사 프로젝트가 없어 지출이 '내부업무(공통비)'로 새면
  //      제품 원가가 회계에서 사라진다. 제품 1건 = 프로젝트 1건으로 만들어 원가를 쌓는다.
  //  어디에: MyDesk /api/projects/create-internal (team=두낫띵 → 업무스케줄2026_미스터두낫띵 DB,
  //      client 자동 '미스터두낫띵'). MyDesk 가 '[내부]' 프리픽스를 붙이는데 이게 오히려 맞다 —
  //      미수금 집계·정산 이상치에서 제외되고(자체제품은 받을 돈이 없다), business 지출 매칭은
  //      프리픽스를 걸러내지 않아 정산행 연결에는 영향이 없다.
  //  매출액 0 이라 정산 자동 동기화(매출>0 조건) 대상이 아니다 →
  //      첫 지출 승인 때 사업화팀이 '➕정산행 생성'(프로젝트당 1행)으로 만들면 그 행에 원가가 누적된다.
  //  멱등: 이미 연결돼 있으면 그대로 반환. 같은 이름 프로젝트가 MyDesk 에 있으면 만들지 않고 연결만.
  //  공용 헬퍼 (2026-08-18): goods UI 버튼과 MyDesk 서버간 호출이 같은 멱등 로직을 쓴다.
  //  반환: { status, body } — status 200 이면 body = { ok, created, reused?, project, pipeline? }
  async function _resolveSettlementProject(pipelineId, ym0, byWho) {
    const p = store.getProject(pipelineId);
    if (!p) return { status: 404, body: { error: 'not found' } };

    const existing = p.links && p.links.mydesk_project;
    if (existing && existing.id) return { status: 200, body: { ok: true, created: false, reused: 'link', project: existing } };

    const base = (process.env.MYDESK_API_URL || 'https://mydesk.jeisha.kr').replace(/\/$/, '');
    // 이름 규칙: [MDN] 제품명 (발주 YYYY-MM) — MyDesk 가 앞에 '[내부] ' 를 덧붙인다
    const ym = String(ym0 || '').match(/^\d{4}-\d{2}$/) ? ym0 : new Date().toISOString().slice(0, 7);
    const wanted = `[MDN] ${String(p.name || '').trim()} (발주 ${ym})`;

    // ① 같은 이름이 이미 있으면 재사용 (버튼 두 번 눌림·다른 세션 생성 대비)
    const md = await fetchMydeskProjects(true);
    if (!md.err) {
      const hit = md.rows.find(x => String(x.name || '').includes(wanted));
      if (hit) {
        const linked = { id: hit.id, name: hit.name, client: hit.client || '', status: hit.status || '', linked_at: new Date().toISOString().slice(0, 10) };
        const saved = store.updateProject(p.id, { links: { ...(p.links || {}), mydesk_project: linked } }, byWho);
        store.addLog(p.id, `정산 프로젝트 연결(기존 재사용): ${hit.name}`, byWho);
        return { status: 200, body: { ok: true, created: false, reused: 'name', project: linked, pipeline: saved } };
      }
    }
    ...
    let created;
    try {
      const resp = await fetch(`${base}/api/projects/create-internal`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: wanted, team: '두낫띵', estimatedDays: 1, dailyHours: 8, assignees: [] }),
        signal: ctrl.signal
      });
      clearTimeout(to);
      created = await resp.json().catch(() => ({}));
      if (!resp.ok) return { status: 502, body: { error: 'MyDesk 프로젝트 생성 실패: ' + (created.error || resp.status) } };
    } catch (e) {
      clearTimeout(to);
      return { status: 502, body: { error: 'MyDesk 연결 실패: ' + e.message } };
    }

    const proj = created.project || created;
    const newId = proj.id || created.id;
    if (!newId) return { status: 502, body: { error: 'MyDesk 응답에 프로젝트 id 가 없습니다' } };

    const linked = {
      id: newId, name: proj.name || wanted, client: proj.client || '미스터두낫띵',
      status: proj.status || '착수 대기중', linked_at: new Date().toISOString().slice(0, 10), created_by_goods: true
    };
    const saved = store.updateProject(p.id, { links: { ...(p.links || {}), mydesk_project: linked } }, byWho);
    store.addLog(p.id, `정산 프로젝트 생성·연결: ${linked.name}`, byWho);
    _mdProjects.at = 0;                                   // 캐시 무효화 — 방금 만든 건이 다음 검색에 뜨게
    return { status: 200, body: { ok: true, created: true, project: linked, pipeline: saved } };
  }

  r.post('/:id/settlement-project', async (req, res) => {
    const out = await _resolveSettlementProject(req.params.id, (req.body || {}).ym, who(req));
    res.status(out.status).json(out.body);
  });

  // ── MyDesk 결제요청 → 정산 프로젝트 해석 (2026-08-18, 경로 통일) — X-API-Key=MYDESK_TO_GOODS_API_KEY ──
  //  MyDesk 에서 "제품제작 + 발주 건 연결"로 접수하면 goods 화면의 대금요청과 동일하게
  //  [MDN] 제품별 프로젝트로 귀속되도록, 링크 조회·이름 재사용·생성을 한 번에 처리 (멱등).
  //  auth.js isPublicPath 등록, 키 가드는 여기서 (expense-record 패턴)
  r.post('/:id/expense-settlement-project', async (req, res) => {
    const expected = process.env.MYDESK_TO_GOODS_API_KEY || '';
    if (!expected) return res.status(503).json({ error: 'not configured (MYDESK_TO_GOODS_API_KEY)' });
    if (!security.keyEq(req.headers['x-api-key'], expected)) return res.status(401).json({ error: 'invalid api key' });
    const out = await _resolveSettlementProject(req.params.id, (req.body || {}).ym, (req.body || {}).by || 'mydesk');
    res.status(out.status).json(out.body);
  });
  ...
  r.post('/hooks/inbound', (req, res) => {
    ...
    const allowed = [process.env.GOODS_API_KEY, process.env.PARTNER_API_KEY, INV_KEY].filter(Boolean);
    const key = req.headers['x-api-key'] || ''; // 2026-08-18 보안: 쿼리스트링 키 제거(로그 노출 방지) — 헤더 전용
    if (!allowed.length || !allowed.some(k => security.keyEq(key, k))) return res.status(401).json({ error: 'unauthorized' });
    const b = req.body || {};
