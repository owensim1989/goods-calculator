...
function router(deps = {}) {
  ...
  r.post('/:id/settlement-project', async (req, res) => {
    ...
    if (existing && existing.id) return res.json({ ok: true, created: false, reused: 'link', project: existing });

    const base = (process.env.MYDESK_API_URL || 'https://mydesk.jeisha.kr').replace(/\/$/, '');
    // 이름 규칙: [MDN] 제품명 (발주 YYYY-MM) — MyDesk 가 앞에 '[내부] ' 를 덧붙인다
    const ym = String((req.body || {}).ym || '').match(/^\d{4}-\d{2}$/)
      ? req.body.ym
    ...
    if (!md.err) {
      ...
      if (hit) {
        ...
        const saved = store.updateProject(p.id, { links: { ...(p.links || {}), mydesk_project: linked } }, who(req));
        store.addLog(p.id, `정산 프로젝트 연결(기존 재사용): ${hit.name}`, who(req));
        return res.json({ ok: true, created: false, reused: 'name', project: linked, pipeline: saved });
      }
    }
    ...
    try {
      ...
      if (!resp.ok) return res.status(502).json({ error: 'MyDesk 프로젝트 생성 실패: ' + (created.error || resp.status) });
    } catch (e) {
      clearTimeout(to);
      return res.status(502).json({ error: 'MyDesk 연결 실패: ' + e.message });
    }

    const proj = created.project || created;
    const newId = proj.id || created.id;
    if (!newId) return res.status(502).json({ error: 'MyDesk 응답에 프로젝트 id 가 없습니다' });
    ...
    const saved = store.updateProject(p.id, { links: { ...(p.links || {}), mydesk_project: linked } }, who(req));
    store.addLog(p.id, `정산 프로젝트 생성·연결: ${linked.name}`, who(req));
    _mdProjects.at = 0;                                   // 캐시 무효화 — 방금 만든 건이 다음 검색에 뜨게
    res.json({ ok: true, created: true, project: linked, pipeline: saved });
  });
