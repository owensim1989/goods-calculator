...
function router(deps = {}) {
  ...
  r.get('/po-defaults', (req, res) => {
    ...
    const shape = (p) => {
      const po = p.po || {};
      const total = (Number(po.qty) && Number(po.unit)) ? Number(po.qty) * Number(po.unit) : null;
      ...
      return {
        po: { cur: po.cur || null, memo: po.memo || '', qty: Number(po.qty) || null, unit: Number(po.unit) || null, total },
        payments: pays,
