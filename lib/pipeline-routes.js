...
function router(deps = {}) {
  ...
  r.post('/hooks/inbound', (req, res) => {
    const expected = process.env.GOODS_API_KEY || process.env.PARTNER_API_KEY || '';
    ...
    const received = Number(b.received != null ? b.received : b.qty) || 0;
    const warehouses = b.warehouse ? [{ code: String(b.warehouse), qty: received }] : [];
    const updated = store.recordInbound(p.id, {
